...
// define our object used for storing errors. All fields are optional(?)
interface StreamWorkerErrorResult {
  errorMessage?: string;
  errorNum?: any;
  errorCode?: number;
}
...
export async function responseProvider(request: EW.ResponseProviderRequest) {
  ...
  */
  ...
  if (
    ...
    try {
      const response = await publishMessageToStream(
        STREAM_APP_NAME,
        STREAM_NAME,
        authorizationHeader,
        body
      ...
      let errorMessage: StreamWorkerErrorResult = {};
